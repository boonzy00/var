...
hyperfine --warmup 3 --min-runs 10 './zig-out/bin/var-bench' \
...
# Calculate speedup
SCALAR_MEAN=$(jq -r '.results[0].mean' scalar-results.json)
SIMD_MEAN=$(jq -r '.results[0].mean' simd-results.json)
